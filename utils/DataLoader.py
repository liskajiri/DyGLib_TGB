...
from .arxiv_dataset import TemporalOGBDataset
import torch
...
def get_link_prediction_tgb_data(dataset_name: str):
    ...
    assert (
        src_node_ids.min() == 0 or dst_node_ids.min() == 0
    ), "Node index should start from 0!"
    assert (
        edge_ids.min() == 0 or edge_ids.min() == 1
    ...
    node_raw_features = np.vstack(
        [np.zeros(node_raw_features.shape[1])[np.newaxis, :], node_raw_features]
    )
    ...
    assert (
        MAX_FEAT_DIM >= node_raw_features.shape[1]
    ), f"Node feature dimension in dataset {dataset_name} is bigger than {MAX_FEAT_DIM}!"
    assert (
        MAX_FEAT_DIM >= edge_raw_features.shape[1]
    ), f"Edge feature dimension in dataset {dataset_name} is bigger than {MAX_FEAT_DIM}!"
...
def get_node_classification_tgb_data(dataset_name: str):
    ...
    for node_label_time in tqdm(label_dict.keys()):
        for src_node_id in label_dict[node_label_time].keys():
            ...
            converted_label_dict[(node_label_time, src_node_id)] = label_dict[
                node_label_time
            ][src_node_id]
    ...
    if os.path.exists(f"./saved_labeled_node_interaction_indices/{dataset_name}.npy"):
        ...
        for node_label_time, src_node_id in tqdm(
            converted_label_dict.keys(), desc=f"data preprocessing for {dataset_name}"
        ):
            ...
            if len(edge_ids[nodes_historical_interactions_mask]) > 0:
                nodes_most_recent_interaction_idx = edge_ids[
                    nodes_historical_interactions_mask
                ][-1]
                assert (
                    nodes_most_recent_interaction_idx
                    == np.where(nodes_historical_interactions_mask)[0][-1]
            ...
            labeled_node_interaction_indices[(node_label_time, src_node_id)] = (
                nodes_most_recent_interaction_idx
            )
    ...
    assert (
        min_val_time > node_interact_times[train_mask].max()
    ), "Train data and validation data are mixed!"
    assert (
        min_test_time > node_interact_times[val_mask].max()
    ...
    full_data = Data(
        ...
        node_label_times=node_label_times,
    )
    train_data = Data(
        src_node_ids=src_node_ids[train_mask],
        dst_node_ids=dst_node_ids[train_mask],
        ...
        edge_ids=edge_ids[train_mask],
        labels=labels[train_mask],
        interact_types=interact_types[train_mask],
        node_label_times=node_label_times[train_mask],
    )
    val_data = Data(
        src_node_ids=src_node_ids[val_mask],
        dst_node_ids=dst_node_ids[val_mask],
        ...
        edge_ids=edge_ids[val_mask],
        labels=labels[val_mask],
        interact_types=interact_types[val_mask],
        node_label_times=node_label_times[val_mask],
    )
    test_data = Data(
        src_node_ids=src_node_ids[test_mask],
        dst_node_ids=dst_node_ids[test_mask],
        ...
        edge_ids=edge_ids[test_mask],
        labels=labels[test_mask],
        interact_types=interact_types[test_mask],
        node_label_times=node_label_times[test_mask],
    )
...
def get_node_classification_ogb(
    dataset_name: str = "ogbn-arxiv", use_messages: bool = True
):
    arxiv_temporal = TemporalOGBDataset(name=dataset_name)

    src_node_ids = arxiv_temporal.full_data.src.numpy()
    dst_node_ids = arxiv_temporal.full_data.dst.numpy()

    node_interact_times = arxiv_temporal.full_data.t.numpy()
    edge_ids = np.arange(len(src_node_ids))
    node_raw_features = arxiv_temporal.data.x.numpy()
    if use_messages:
        edge_raw_features = arxiv_temporal.full_data.msg.numpy()
    else:
        edge_raw_features = np.ones((len(src_node_ids), 1))

    train_mask = arxiv_temporal.train_mask
    val_mask = arxiv_temporal.val_mask
    test_mask = arxiv_temporal.test_mask
    eval_metric_name = arxiv_temporal.eval_metric
    num_classes = arxiv_temporal.num_classes

    labels = torch.nn.functional.one_hot(
        arxiv_temporal.data.y[src_node_ids].flatten()
    ).numpy()
    node_label_times = copy.deepcopy(node_interact_times)

    train_interact_types = np.array(["train" for _ in range(train_mask.sum())])
    val_interact_types = np.array(["validate" for _ in range(val_mask.sum())])
    test_interact_types = np.array(["test" for _ in range(test_mask.sum())])
    interact_types = np.concatenate(
        [train_interact_types, val_interact_types, test_interact_types]
    )
