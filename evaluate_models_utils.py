        ...
        for batch_idx, evaluate_data_indices in enumerate(
            ...
            for sample_idx in range(len(batch_src_node_ids)):
                ...
                input_dict = {
                    ...
                    "y_pred_neg": negative_probabilities[
                        sample_idx * num_negative_samples_per_node : (sample_idx + 1)
            ...
            evaluate_idx_data_loader_tqdm.set_description(
                f"{eval_stage} for the {batch_idx + 1}-th batch"
            )
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(
            ...
            (
                ...
                evaluate_data.interact_types[evaluate_data_indices],
            ...
            if eval_stage == "val":
                ...
                assert (
                    eval_stage == "test"
                ), f"Wrong setting of eval_stage {eval_stage}!"
            ...
            if len(eval_idx) > 0:
                ...
                predicts = model.node_classifier(x=batch_src_node_embeddings).squeeze(
                    dim=-1
                )
                ...
                for idx in eval_idx:
                    evaluate_predicts_per_timeslot_dict[
                        batch_node_label_times[idx]
                    ].append(predicts[idx].softmax(dim=0).cpu().detach().numpy())
                    evaluate_labels_per_timeslot_dict[
                        batch_node_label_times[idx]
                    ].append(labels[idx].cpu().detach().numpy())

                # evaluate_idx_data_loader_tqdm.set_description(
                #     f"{eval_stage} for the {batch_idx + 1}-th batch, loss: {loss.item()}"
                # )
        ...
        for time_slot in tqdm(evaluate_predicts_per_timeslot_dict):
            time_slot_predictions = np.stack(
                evaluate_predicts_per_timeslot_dict[time_slot], axis=0
            )
            time_slot_labels = np.stack(
                evaluate_labels_per_timeslot_dict[time_slot], axis=0
            )
            if isinstance(evaluator, OGBEvaluator):
...
def evaluate_edge_bank_link_prediction(
    ...
    val_data: Data,
    test_data: Data,
    val_idx_data_loader: DataLoader,
    ...
    train_val_data = Data(
        src_node_ids=np.concatenate(
            [train_data.src_node_ids, val_data.src_node_ids], axis=0
        ),
        dst_node_ids=np.concatenate(
            [train_data.dst_node_ids, val_data.dst_node_ids], axis=0
        ),
        node_interact_times=np.concatenate(
            [train_data.node_interact_times, val_data.node_interact_times], axis=0
        ),
    ...
    for run in range(args.num_runs):
        ...
        if args.edge_bank_memory_mode == "time_window_memory":
            ...
            args.save_result_name = (
                f"eval_{args.model_name}_{args.edge_bank_memory_mode}_seed{args.seed}"
            )
        ...
        for batch_idx, val_data_indices in enumerate(val_idx_data_loader_tqdm):
            ...
            batch_src_node_ids, batch_dst_node_ids, batch_node_interact_times = (
                val_data.src_node_ids[val_data_indices],
                val_data.dst_node_ids[val_data_indices],
            ...
            negative_edges = (
                repeated_batch_src_node_ids,
                batch_neg_dst_node_ids.flatten(),
            )
            ...
            history_data = Data(
                src_node_ids=np.concatenate(
                    [
                        ...
                        val_data.src_node_ids[: val_data_indices[0]],
                    ],
                    axis=0,
                ),
                dst_node_ids=np.concatenate(
                    [
                        ...
                        val_data.dst_node_ids[: val_data_indices[0]],
                    ],
                    axis=0,
                ),
                node_interact_times=np.concatenate(
                    [
                        ...
                        val_data.node_interact_times[: val_data_indices[0]],
                    ],
                    axis=0,
                ),
                edge_ids=np.concatenate(
                    [train_data.edge_ids, val_data.edge_ids[: val_data_indices[0]]],
                    axis=0,
                ),
                labels=np.concatenate(
                    [train_data.labels, val_data.labels[: val_data_indices[0]]], axis=0
                ),
            )
            ...
            val_idx_data_loader_tqdm.set_description(
                f"validate for the {batch_idx + 1}-th batch"
            )
        ...
        for batch_idx, test_data_indices in enumerate(test_idx_data_loader_tqdm):
            ...
            test_idx_data_loader_tqdm.set_description(
                f"test for the {batch_idx + 1}-th batch"
            )
...
def evaluate_parameter_free_node_classification(
    args: argparse.Namespace,
    train_data: Data,
    val_data: Data,
    test_data: Data,
    train_idx_data_loader: DataLoader,
    val_idx_data_loader: DataLoader,
    test_idx_data_loader: DataLoader,
    eval_metric_name: str,
    num_classes: int,
    ...
    for run in range(args.num_runs):
        ...
        if args.model_name == "PersistentForecast":
            ...
            assert (
                args.model_name == "MovingAverage"
            ), f"Wrong setting of model_name {args.model_name}!"
            model = MovingAverage(
                num_classes=num_classes, window_size=args.moving_average_window_size
            )
        ...
        def inner_evaluate_parameter_free_node_classification(
            evaluate_idx_data_loader: DataLoader, evaluate_data: Data, stage: str
        ):
            """
            ...
            for batch_idx, evaluate_data_indices in enumerate(
                ...
                if stage == "train":
                    eval_idx = torch.tensor(
                        np.where(batch_interact_types == "train")[0]
                    )
                    ...
                    eval_idx = torch.tensor(
                        np.where(batch_interact_types == "validate")[0]
                    )
                ...
                if len(eval_idx) > 0:
                    ...
                    for idx in eval_idx:
                        ...
                        model.update_memory(
                            node_id=batch_src_node_ids[idx],
                            node_label=batch_labels[idx],
                        )
                    ...
                    evaluate_losses.append(loss.item())
                    # append the predictions and labels to evaluate_predicts_per_timeslot_dict and evaluate_labels_per_timeslot_dict
                    for predict, label_time, label in zip(
                        predicts, label_times, labels
                    ):
                        evaluate_predicts_per_timeslot_dict[label_time].append(
                            predict.numpy()
                        )
                        evaluate_labels_per_timeslot_dict[label_time].append(
                            label.numpy()
                        )

                    evaluate_idx_data_loader_tqdm.set_description(
                        f"{stage} for the {batch_idx + 1}-th batch, loss: {loss.item()}"
                    )
        ...
        train_losses, train_metrics = inner_evaluate_parameter_free_node_classification(
            evaluate_idx_data_loader=train_idx_data_loader,
            evaluate_data=train_data,
            stage="train",
        )

        val_losses, val_metrics = inner_evaluate_parameter_free_node_classification(
            evaluate_idx_data_loader=val_idx_data_loader,
            evaluate_data=val_data,
            stage="val",
        )

        test_losses, test_metrics = inner_evaluate_parameter_free_node_classification(
        ...
        result_json = {
            "train metrics": {
                metric_name: f"{train_metric_dict[metric_name]:.4f}"
                for metric_name in train_metric_dict
            },
            "validate metrics": {
                metric_name: f"{val_metric_dict[metric_name]:.4f}"
                for metric_name in val_metric_dict
    ...
    for metric_name in train_metric_all_runs[0].keys():
        logger.info(
            f"train {metric_name}, {[train_metric_single_run[metric_name] for train_metric_single_run in train_metric_all_runs]}"
        )
        logger.info(
            f"average train {metric_name}, {np.mean([train_metric_single_run[metric_name] for train_metric_single_run in train_metric_all_runs]):.4f} "
...
def convert_ogb_predictions_from_one_hot(
    time_slot_labels: torch.Tensor, time_slot_predictions
) -> np.array:
    # NOTE: labels are one hot, but OGB requires classical labels
    time_slot_labels = np.argmax(time_slot_labels, axis=1).reshape((-1, 1))
    time_slot_predictions = np.argmax(time_slot_predictions, axis=1).reshape((-1, 1))
    return time_slot_labels, time_slot_predictions
